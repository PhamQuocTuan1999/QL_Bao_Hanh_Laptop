...
class bhController extends Controller
{
        ...
        public function bh_day()
    {
        ...
        INNER JOIN khachhang as kh on kh.id = hd.kh_ma
           ...
           WHERE hd.hd_taoMoi >= DATE_SUB(NOW(), INTERVAL 1 day) AND hd.lhd_ma=1 AND hd.hd_trangThai >=0
        '
    ...
    public function bh_week()
    {
       ...
       SELECT  nv.id, nv.nv_hoTen , r.name FROM nhanvien AS nv
        ...
        INNER JOIN khachhang as kh on kh.id = hd.kh_ma
              INNER JOIN loai_hoa_don as lhd on lhd.lhd_ma = hd.lhd_ma
           WHERE hd.hd_taoMoi >= DATE_SUB(NOW(), INTERVAL 7 day) AND hd.lhd_ma=1 AND hd.hd_trangThai >=0
        '
    );
    return view('admin.QuanLy.Day.BH')->with('HoaDon', $HoaDon)->with('nv', $nv);
    }
    public function bh_month()
    {
        $nv = DB::select(
            '
       SELECT  nv.id, nv.nv_hoTen , r.name FROM nhanvien AS nv
        ...
        INNER JOIN khachhang as kh on kh.id = hd.kh_ma
              INNER JOIN loai_hoa_don as lhd on lhd.lhd_ma = hd.lhd_ma
           WHERE hd.hd_taoMoi >= DATE_SUB(NOW(), INTERVAL 30 day) AND hd.lhd_ma=1 AND hd.hd_trangThai >=0
        '
    );
    return view('admin.QuanLy.Day.BH')->with('HoaDon', $HoaDon)->with('nv', $nv);
    }
